#!/usr/bin/env python3
"""
Plot RNN training BLEU curves from *_history.log files.
Output:
...
def main():
    logs = sorted(glob("rnn_*_history.log"))
    if not logs:
        print("No rnn_*_history.log files found.")
        return
    ...
    if not plotted:
        print("No BLEU entries found in RNN logs.")
        return

    ax.set_title("RNN Ablations: BLEU over Epochs")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("BLEU")
    ...
    plt.tight_layout()
    out_path = "rnn_train_bleu_over_epochs.png"
