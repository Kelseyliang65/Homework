#!/usr/bin/env python3
"""
Evaluate all RNN checkpoints on the test set and compare results.
Outputs:
  - rnn_test_results/summary.csv
  - rnn_test_results/bleu_comparison.png
  - rnn_test_results/sample_comparisons.jsonl
  - rnn_test_results/<model_tag>_test_results.jsonl
"""
import argparse
import json
import os
...
from inference import load_rnn, translate_sentence
...
def list_rnn_checkpoints(ckpt_dir: str) -> List[str]:
    return sorted(
        [
            os.path.join(ckpt_dir, f)
            for f in os.listdir(ckpt_dir)
            if f.startswith("rnn_") and f.endswith("_best.pt")
        ]
    )
...
def plot_bleu(summary: List[Dict[str, str]], out_path: str) -> None:
    ...
    ax1.set_title("RNN Test BLEU (Line)")
    ax1.set_ylabel("BLEU")
    ...
    ax2.set_title("RNN Test BLEU (Bar)")
    ax2.set_ylabel("BLEU")
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="rnn_test_results")
    ...
    rnn_ckpts = list_rnn_checkpoints(args.ckpt_dir)
    if not rnn_ckpts:
        raise RuntimeError("No rnn_*_best.pt checkpoints found.")
    ...
    for ckpt in rnn_ckpts:
        tag = model_tag_from_ckpt(ckpt)
        model, zh_vocab, en_id2w = load_rnn(ckpt, device)
